...
class ScoringStrategy:
    """Business class responsible for computing and updating players' Elo ratings."""
    ...
    def update_player_ratings(cls, game: Game):
        """Calculates and updates the elo attributes of the players involved in a game.
        No update if there is no winner (Draw).
 
        Args:
            game (Game): The game containing the two players and the winner.
        """
        if not game.winner_name:
            return
 
        p1 = game.player1
        p2 = game.player2
 
        p1.elo, p2.elo = cls.calculate_new_ratings(p1.elo, p2.elo, player_a_won=(p1 == game.winner_name))